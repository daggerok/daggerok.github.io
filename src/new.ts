...
if (process.argv.length <= 2) {
  ...
  console.log(`
    Usage:
      bun new ${year} ${year}-${month}-${day}-Hello-World
  `);
  process.exit(1);
}

const [exec, script, ...rest] = process.argv;
const joinedString = rest.join(' ');

const blogDir = path.join(__dirname, 'blog', ...rest);
fs.mkdirpSync(blogDir);

const blobPost = path.join(blogDir, 'README.md');
fs.writeSync(fs.openSync(blobPost, 'w'), `# TODO ${joinedString}
${joinedString}
